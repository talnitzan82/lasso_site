...
while($row=mysql_fetch_assoc($query)) {
	...
	# set pictures
	$array2 = array();
	$sql2   = "SELECT * FROM gallery WHERE sub = '{$row['id']}' AND hide!='yes' ORDER BY logo DESC,sort DESC";
	...
	$array2 = array();
	$sql2   = "SELECT * FROM gallery WHERE sub = '{$row['id']}' AND logo = 'yes' LIMIT 1";
	$query2 = $database->query($sql2);
	while($row2=mysql_fetch_assoc($query2)) {		
		$array2[] = 'http://www.lasso.co.il/'.$row2['image'];			
	}	
	$array['details']['logo'] = $array2;
