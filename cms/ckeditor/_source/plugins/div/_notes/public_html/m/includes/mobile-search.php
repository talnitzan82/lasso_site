...
<form class="jNice no-margin clearfix">
                <ul class="search-options-col clearfix">
					...
					while($row=mysql_fetch_assoc($query)) {
						$array1[]=$row;
						$counter2 = 0;
						unset($array2,$array2_ids,$all_links,$level2);
                    ...
                    <li class="m-options-col">
				        ...
				        foreach ($array2 as $key=>$value) {							
                            ...
                            $link = HandleSearch(str_replace("\\","0",urlencode($value['name'])),$params);
                            ...
                            ?>
                            <div class="span6"><a href="<?=$link?>" class="idx-buttons-search idx-buttons idx-buttons-<? if (in_array(str_replace(array(" ","\\"),array("_","0"),$value['name']),$params)){ ?>b<? } else { ?>a<? } ?>"><?=$value['name']?> <? if (in_array(str_replace(array(" ","\\"),array("_","0"),$value['name']),$params)){ ?><img style="position:absolute; right:10px; top:15px;;" src="../../img/x_icon.png" alt="" /><? } ?></a></div>
                            <?			 
                            if ($k==1) 
                            {
                            ?>
                        ...
                        if ($row['type']=='0') {
							/*
                        ?>
                        <div class="dark-gray dark-gray4" style="text-align:right; clear:both;"><a href="<?=$link?>">בחר הכל</a></div>
                        <?
						*/
                        }
                        ?>
