...
if ($_SERVER['REQUEST_URI']!='') {
	...
	if ($page['id']=='') {
		...
		if ($page['template'] == 'product.php?id=1' && preg_match('/מפה/',urldecode($find[0]))){
			$page['template'] = 'product_map.php?id=1';
		}
		
		if ($page['template'] == 'product.php?id=1' && preg_match('/שלח/',urldecode($find[0]))){
			$page['template'] = 'product_send.php?id=1';
		}
		
		if ($page['id']=='') {
			$url_explode = explode("-",$database->mysql_prep($find[0]));
			$url_implde = str_replace(array("_","0"),array(" ","\\\\"),urldecode(implode("','",$url_explode)));
			$sql = "SELECT * FROM categories WHERE name in ('$url_implde')";
			$page = mysql_fetch_array($database->query($sql));	
			if ($page['id']!='') {
				$page['template'] = 'list.php?id=1';
			}			
		}		
	}
...
if (!isset($page)) {	
	...
	$templates[] = 'product_map.php';
	$templates[] = 'product_send.php';
