...
(function (factory) {
  ...
  Wookmark = (function(options) {
      ...
      for (i = 0; i < this.placeholders.length; i++) {
        ...
        if (i >= columnsLength || !column[column.length - 1]) {
          ...
          $placeholder.css({
            ...
            right: i * columnWidth + sideOffset,
            top: top,
    ...
    Wookmark.prototype.layout = function() {
      ...
      this.direction = this.align == 'right' ? 'right' : 'right';
    ...
    Wookmark.prototype.layoutFull = function(columnWidth, columns, offset) {
      var item, top, right, i = 0, k = 0 , j = 0,
          activeItems = this.getActiveItems(),
          length = activeItems.length,
          shortest = null, shortestIndex = null,
          itemCSS = {position: 'absolute'},
          sideOffset, heights = [],
          rightAligned = this.align == 'right' ? true : false;
      ...
      for (; i < length; i++ ) {
        ...
        // stick to right side if alignment is right and this is the first column
        if (shortestIndex == 0 && rightAligned) {
          sideOffset = 0;
        } else {
		...
		/// TAL MODIFICATION
		//itemCSS.transition = "top 1s, right 1s";
       // if (page_load_wookmark==1) {
			//$item.animate({ right: sideOffset +"px",top: shortest+"px"}); 
